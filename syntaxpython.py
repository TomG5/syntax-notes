...
{Jan, Fev, Mar}     : Set           # Inalterável após definido, mas pode receber novas entradas / Somente valores únicos
...
drop()      : remove índice, coluna ou valores de uma matriz (colocando 'inplace:True' como parâmetro efetiva a mudança)
...
index()     : retorna o índice de um dado na variável
...
isupper()     : valida letras maiúsculas
islower()     : valida letras minúsculas
title()       : todas as primeiras letras das palavras maiúsculas
strip()       : remover espaços
join()        : juntar palavras # Sintaxe < *** separador ***.join(lista) >
split()       : separar palavras da mesma string
replace()     : substituir caracter
find()        : retorna o índice do string procurado na palavra
capitalize()  : mantém apenas a primeira letra da string em maiúsculo
...
append()      : adiciona um valor em lista
extend()      : adiciona uma lista dentro de outra
insert()      : adiciona dado em lista em dada posição
remove()      : remover item de lista por valor
pop()         : remover item de lista por índice (caso em branco remove o último)
del()         : remover item de lista por índice ou slice
clear()       : limpa toda a lista
sort()        : ordena itens de uma lista (uso após a variável)
sorted()      : ordena itens de uma lista (uso sobre a variável)
zip()         : integra múltiplas listas com base nos índices de seus elementos
count()       : conta a frequência de dado elemento numa lista
len()         : conta quantos elementos tem numa lista
index()       : retorna índice de item em lista
reverse()     : reverte a ordem da lista
...
new_list = [x for x in some_iterable if *** condição ***]
...
update()  : adiciona/atualiza dados a um dicionário
pop()     : remove item do dicionário
del()     : remove item do dicionário
clear()   : limpa todo o dicionário
zip()     : integra duas listas em um dicionário
get()     : procura dada chave e retorna valor booleano
keys()    : retorna chaves do dicionário
values()  : retorna valores do dicionário
items()   : retorna conjuntos de chaves e valores do dicionário (transformar em lista para pesquisar por índice)

    ## Dictionary Comprehensions (Criação de dicionários a partir de objetos iteráveis):

my_dict = {key: value if *** condição *** for element in some_iterable}
'''
...
add()       : adiciona novos valores
update()    : adiciona novos valores
len()       : retorna tamanho do set
remove()    : remove elemento do set
clear()     : limpa os elementos do set
...
new_set = [x for x in some_iterable]

new_set = [x for x in some_iterable if *** condição ***]
...
tupla[2] = retorna o valor de índice 2 na tupla
'''
